...
const StatusIcon = ({ status, hasResult }: { status: string, hasResult: boolean }) => {
  ...
  return <div className="w-4 h-4 rounded-full border border-slate-400 dark:border-slate-600" />;
};
...
export const AnalysisSection: React.FC<{ 
  ...
  return (
    <div className={`bg-slate-50/50 dark:bg-slate-800/50 rounded-xl p-4 border transition-colors duration-500 ${result ? 'border-purple-500/30' : 'border-slate-200 dark:border-slate-700/50'}`}>
        <div className="flex items-center justify-between mb-2">
            ...
            {result && (
                <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700">
                    ...
                    <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400">{modelName}</span>
                </div>
            )}
        </div>
        {result ? (
...
export const PromptSection: React.FC<{
  ...
  modelName?: string;
   ...
   const PromptBox = ({ title, text, colorClass, borderColor }: { title: string; text: string; colorClass: string; borderColor: string }) => (
    <div className={`relative bg-white dark:bg-slate-900/50 rounded-xl border ${borderColor} p-3 group flex flex-col h-40 transition-all hover:bg-slate-50 dark:hover:bg-slate-900 shadow-sm dark:shadow-none`}>
        ...
        <div className="flex-1 overflow-hidden relative">
             <p className="text-slate-600 dark:text-slate-400 text-xs font-mono leading-relaxed break-words whitespace-pre-wrap line-clamp-5">
                {text}
            </p>
        </div>
        <div className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
            <button 
                ...
                className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 p-1.5 rounded-md shadow-sm transition-colors"
                title="复制文本"
            >
    ...
    <>
        <div className="space-y-2">
            ...
            {result ? (
                ...
                <div className="grid grid-cols-2 gap-4">
                        <div className={`h-40 bg-slate-100 dark:bg-slate-800/30 rounded-xl border border-slate-200 dark:border-slate-700/30 flex items-center justify-center ${status === 'analyzing' ? 'animate-pulse' : ''}`}>
                            <span className="text-slate-400 dark:text-slate-600 text-xs">Waiting...</span>
                        </div>
                        <div className={`h-40 bg-slate-100 dark:bg-slate-800/30 rounded-xl border border-slate-200 dark:border-slate-700/30 flex items-center justify-center ${status === 'analyzing' ? 'animate-pulse' : ''}`}>
                             <span className="text-slate-400 dark:text-slate-600 text-xs">Waiting...</span>
                        </div>
        ...
        {modalContent && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl">
                    <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800">
                        <h3 className="font-bold text-slate-900 dark:text-white">{modalContent.title}</h3>
                        <button 
                    ...
                    <div className="p-6 overflow-y-auto custom-scrollbar">
                        <p className="text-slate-700 dark:text-slate-300 font-mono text-sm leading-relaxed whitespace-pre-wrap">
                            {modalContent.text}
                        </p>
                    </div>
...
export const ImagePreviewSection: React.FC<{
    ...
    return (
        <div className="space-y-2">
            ...
            <div className="relative aspect-video w-full rounded-xl overflow-hidden bg-slate-100 dark:bg-black ring-1 ring-slate-200 dark:ring-white/10 shadow-2xl group">
                {generatedImage ? (
                <>
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 p-8 text-center bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800/50">
                    {status === 'generating_image' ? (
                        <>
                            <div className="relative">
                                <div className="w-12 h-12 rounded-full border-4 border-slate-200 dark:border-slate-800 border-t-emerald-500 animate-spin mb-4" />
                                <div className="absolute inset-0 flex items-center justify-center">
                                    <Sparkles className="w-5 h-5 text-emerald-500/50" />
                                </div>
                            </div>
                            <p className="text-slate-600 dark:text-white font-medium animate-pulse">
                                正在绘制高清封面...
                            </p>
